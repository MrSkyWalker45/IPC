...
const FileCard = ({ file_name, choice,}) => {
    ...
    <View style={tw`flex-1  items-center justify-center`}>
      <TouchableOpacity onPress={() => Linking.openURL(Url)} style={tw.style('pt-4 items-center justify-center')}>
        <Card containerStyle={styles.Card}>
          ...
          <Card.FeaturedSubtitle style={tw.style('text-gray-400 text-xs pr-20 text-center px-2 font-bold')}>{Type}</Card.FeaturedSubtitle>
           <Card.FeaturedSubtitle style={tw.style('text-gray-400 text-xs pr-20 text-center px-2 font-bold')}>{Time}</Card.FeaturedSubtitle>
          <Image source={isPDF ? require('../assets/pdf.png') : {uri: Url }} style={tw.style('h-35 w-40 rounded mx-5 items-cente',{alignSelf:'center'})} />
